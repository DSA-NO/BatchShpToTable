...
def parse_filename(filepath):
    ...
    Run_metadata = namedtuple("Run_metadata",['outputname' , 'timestep' , 'extra'])

    extra = nuclide if nuclide else run_metadata['agegroup']
    key = Run_metadata(outputname = outputname, timestep = timestep, extra= extra)
    metadata_dict[key] = filepath

    return runname, timestamp, metadata_dict, key
...
# pd.DataFrame(columns=metadata['AnlopGrotsund_phase1_250m']['202005081553'].keys())
if __name__ == "__main__":    
    ...
    pp=pp.glob("**/*.shp")
    max_values = {}
    # results = {keys, values for x in parse_filename(x) }
    metadata = {}
    for x in pp:
        runname, timestamp, metadata_dict, key = parse_filename(x)
        metadata.setdefault(runname,{}).setdefault(timestamp,{})
        metadata[runname][timestamp][key] = metadata_dict
 ...
 # %%   
    ...
    if 1:
        ...
        # else:
    print(max_values)
    for key, item in max_values.items():
